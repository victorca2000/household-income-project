...
```{r}
# Extract standard coefficients in ridge fit
extract_std_coefs(ridge_fit, train_data)

# Exports extracted coefficients table into a csv
write.csv(extract_std_coefs(ridge_fit, train_data) ,"../results/ridge_coeffs.csv", row.names = FALSE)
```
...
# Exports extracted coefficients table into a csv
write.csv(extract_std_coefs(lasso_fit, train_data) ,"../results/lasso_coeffs.csv", row.names = FALSE)
```
...
extract_std_coefs(elnet_fit_best, train_data)

write.csv(extract_std_coefs(elnet_fit_best, train_data) ,"../results/elnet_coeffs.csv", row.names = FALSE)
```
...
RMSE_table <- tibble("Ols RMSE" = RMSE_ols, "Ridge RMSE" = RMSE_ridge, "Lasso RMSE" = RMSE_lasso, "Elastic Net RMSE" = RMSE_elnet)
RMSE_table

write.csv(RMSE_table, "../results/regr_RMSE.csv", row.names = FALSE)
```
...
write.csv(cp_table, "../results/cp_table.csv", row.names = FALSE)
...
write.csv(optimal_tree_info, "../results/optimal_tree.csv", row.names = FALSE)
